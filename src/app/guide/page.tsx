...
interface CareerSlot {
  ...
  scholarship: string;
  timeline: string;
  expectedFirstSalary: string;
}

const EMPTY_CAREER_SLOT: CareerSlot = {
  title: "",
  degree: "",
  institutionExample: "",
  entranceExam: "",
...
function CountdownTimer({
  ...
  }, [isRunning, hasStarted, timeLeft > 0]); // eslint-disable-line react-hooks/exhaustive-deps

  // Handle timer completion
  useEffect(() => {
    if (timeLeft === 0 && hasStarted && !isComplete) {
      setIsComplete(true);
      onComplete();
  ...
  }, [timeLeft, hasStarted, isComplete, onComplete]);
  ...
  // Color logic
  let timerColor = "text-chocolate";
  let bgColor = "bg-ivory";
  let borderColor = "border-cream";
  let ringColor = "ring-gold/20";
  ...
  return (
    <div className={`rounded-2xl ${bgColor} border ${borderColor} p-6 text-center ring-2 ${ringColor} transition-all duration-500`}>
      {!hasStarted ? (
        <div className="space-y-4">
          ...
          <div>
            <p className="text-3xl font-bold text-chocolate tracking-wider">15:00</p>
            <p className="text-sm text-muted mt-1">Session Timer</p>
          </div>
          <button
            onClick={() => {
              setHasStarted(true);
            ...
            className="px-8 py-3 rounded-xl bg-gold text-white font-semibold text-lg hover:bg-gold-dark transition-all shadow-lg shadow-gold/20"
          >
            Start Session
          </button>
        </div>
        ...
        <div className="space-y-3">
          ...
          <p className={`text-4xl font-bold ${timerColor} tracking-wider`}>00:00</p>
          <p className="text-lg font-semibold text-red-600">Session Complete</p>
          <p className="text-sm text-red-500">Please wrap up and save your notes.</p>
        </div>
      ) : (
        <div className="space-y-2">
          <p className="text-xs font-medium text-muted uppercase tracking-wider">Session In Progress</p>
          <p className={`text-5xl font-bold ${timerColor} tracking-wider font-mono transition-colors duration-500`}>
            {formattedTime}
          </p>
          {timeLeft <= 120 && (
            <p className="text-sm font-medium text-red-500 animate-pulse">Time is almost up!</p>
          )}
          {timeLeft > 120 && timeLeft <= 180 && (
            <p className="text-sm font-medium text-amber-600">3 minutes remaining</p>
          )}
        </div>
      )}
    </div>
...
/* ------------------------------------------------------------------ */
/*  Career Pathway Card Component                                      */
/* ------------------------------------------------------------------ */
function CareerPathwayCard({
  careers,
  onChange,
}: {
  careers: CareerSlot[];
  onChange: (careers: CareerSlot[]) => void;
}) {
  const [expandedSlots, setExpandedSlots] = useState<Record<number, boolean>>({ 0: true });

  const toggleSlot = (index: number) => {
    setExpandedSlots((prev) => ({ ...prev, [index]: !prev[index] }));
  };

  const updateCareer = (index: number, field: keyof CareerSlot, value: string) => {
    const updated = [...careers];
    updated[index] = { ...updated[index], [field]: value };
    onChange(updated);
  };

  const fields: { key: keyof CareerSlot; label: string; placeholder: string }[] = [
    { key: "title", label: "Career Title", placeholder: "e.g. Software Engineer" },
    { key: "degree", label: "Degree Required", placeholder: "e.g. B.Tech Computer Science" },
    { key: "institutionExample", label: "Institution Example", placeholder: "e.g. IIT Delhi, NIT Patna" },
    { key: "entranceExam", label: "Entrance Exam", placeholder: "e.g. JEE Main, JEE Advanced" },
    { key: "scholarship", label: "Scholarship", placeholder: "e.g. AICTE Pragati, State Merit" },
    { key: "timeline", label: "Timeline", placeholder: "e.g. 4 years (B.Tech) + 2 years (M.Tech)" },
    { key: "expectedFirstSalary", label: "Expected First Salary", placeholder: "e.g. 4-8 LPA" },
  ];

  return (
    <div className="space-y-3">
      <h4 className="text-sm font-semibold text-chocolate flex items-center gap-2">
      ...
      {[0, 1, 2].map((index) => {
        ...
        const label = `Career ${index + 1}${isOptional ? " (Optional)" : ""}`;
        const isExpanded = expandedSlots[index] || false;
        const hasData = careers[index] && careers[index].title.trim() !== "";

        return (
          <div key={index} className="rounded-xl border border-gold/20 overflow-hidden">
            {/* Collapsible Header */}
            <button
              onClick={() => toggleSlot(index)}
              className="w-full flex items-center justify-between px-4 py-3 bg-gradient-to-r from-gold/10 to-gold/5 hover:from-gold/15 hover:to-gold/10 transition-all"
            >
              <div className="flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-gold text-white text-xs font-bold flex items-center justify-center">
                  {index + 1}
                </span>
                <span className="text-sm font-semibold text-gold-dark">{label}</span>
                {hasData && (
              ...
              <svg
                className={`w-4 h-4 text-gold-dark transition-transform ${isExpanded ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            {isExpanded && (
              <div className="p-4 bg-white space-y-3">
                {fields.map(({ key, label: fieldLabel, placeholder }) => (
                  <div key={key}>
                    <label className="block text-xs font-medium text-muted mb-1">{fieldLabel}</label>
                    <input
                      type="text"
                      value={careers[index]?.[key] || ""}
                      onChange={(e) => updateCareer(index, key, e.target.value)}
                      placeholder={placeholder}
                      className="w-full px-3 py-2 rounded-lg border border-cream bg-ivory text-chocolate text-sm placeholder:text-muted/60 focus:outline-none focus:ring-2 focus:ring-gold/30 focus:border-gold transition-all"
                    />
                  </div>
                ))}
              </div>
...
/* ------------------------------------------------------------------ */
/*  Session Notes Form (with Timer + Career Pathway Card)              */
/* ------------------------------------------------------------------ */
function SessionNotesForm({
  ...
  // Parse existing pathway card data from metadata
  const existingPathwayData = (() => {
    try {
      const metaPart = session.notes?.split("\n---meta:")[1];
      if (metaPart) {
        const meta = JSON.parse(metaPart);
        if (meta.pathwayCardData) return meta.pathwayCardData;
      }
    } catch {
      // ignore
    }
    return [{ ...EMPTY_CAREER_SLOT }, { ...EMPTY_CAREER_SLOT }, { ...EMPTY_CAREER_SLOT }];
  })();
  ...
  const [careers, setCareers] = useState<CareerSlot[]>(existingPathwayData);
  ...
  const handleTimerComplete = useCallback(() => {
    // Timer hit zero - auto-mark as needing wrap-up
  }, []);

  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await fetch("/api/guide/session-notes", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleGenerateRef = async () => {
    setRefLoading(true);
    try {
      const res = await fetch("/api/guide/reference-slip", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          sessionId: session.session_id,
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {["Waiting", "In Cabin", "In Progress", "Completed", "No Show"].map((s) => (
                <button
                  key={s}
                  onClick={() => {
...
export default function GuidePortal() {
  ...
  useEffect(() => {
    const cookie = document.cookie
      .split("; ")
      .find((c) => c.startsWith("guide_token="));
    if (cookie) {
      try {
  ...
  const handleLogout = () => {
    document.cookie =
      "guide_token=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
    setAuth(null);
    setSessions([]);
  };
